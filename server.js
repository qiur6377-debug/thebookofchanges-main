...
const { buildZhouyiInterpretationMessages } = require('./prompts/zhouyi-interpretation');
...
function isBlockedAnalyticsKey(key) {
  ...
  if (SAFE_ANALYTICS_KEYS.has(key) || SAFE_ANALYTICS_KEYS.has(normalized)) return false;
...
function normalizeShortContextText(value, maxLength = 90) {
  if (typeof value !== 'string') return '';
  return value.trim().replace(/\s+/g, ' ').slice(0, maxLength);
}

function normalizeBackgroundContext(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const scenario = normalizeShortContextText(value.scenario, 24);
  const focus = normalizeShortContextText(value.focus, 40);
  const note = normalizeShortContextText(value.note, 90);
  const summary = normalizeShortContextText(value.summary, 240);
  if (!scenario && !focus && !note && !summary) return null;
  return { scenario, focus, note, summary };
}
...
app.post('/api/interpret', async (req, res) => {
  ...
  const question = normalizeQuestionInput(req.body?.question);
  const backgroundContext = normalizeBackgroundContext(req.body?.backgroundContext);
  ...
  const messages = buildZhouyiInterpretationMessages({
    question,
    hexagram,
    backgroundContext,
    changingPositions,
